 ...
 * 
 * This class now implements IDrawable and ICollider from GameObject Abstract class. This class has method draw from IDrawable that draws the ant object that is a filled red circle. This class
 * inherits the ICollider methods from GameObject. This class inherits the collidesWith method from GameObject to check if the ant has collided with another game object. Then it inherits 
 * handleCollison() from GameObject to handle the collision depending on the game object it has collided.
 * 
 * @author Kenny Nguyen
 * @version A3 Project
 * - CSC 133-03
...
public class Ant extends MovableObject implements ISteerable {
	 ...
	 */
	public Ant(float xLocation, float yLocation, GameWorld gw) {
	...
	public void antReset() {
		this.setHeading(0);
		this.setSpeed(0);
		this.setMaximumSpeed(200);
		this.setLastFlagReached(1);
		this.setXLocation(50);
		this.setYLocation(50);
		this.setHealthLevel(10);
		this.setFoodLevel(2000);
		this.setIsDead(false);
		this.setColor(ColorUtil.rgb(255, 0 ,0));
	}
	...
	/**
	 * This method draw is from the interface IDrawable. For this class, it draws the ant object that is a filled red circle
	 * 
	...
	public void draw(Graphics g, Point pCmpRelPrnt) {
		...
		g.fillArc((int)(pCmpRelPrnt.getX() + this.getXLocation()), (int)(pCmpRelPrnt.getY() + this.getYLocation()), this.getSize(), getSize(), 0, 360); 
	}
