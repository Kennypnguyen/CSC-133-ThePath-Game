...
public class Ant extends MovableObject implements ISteerable {
	...
	public void steerHeading(int headingChange) {
		//If the heading is at 0, then set the current heading to 359 then minus the headingChange because can't have negative heading
		//Used for if the user is turning left at 0 heading (Complete 360 turn)
		if(this.getHeading() == 0) {
			this.setHeading(359 + headingChange);
		}
		//If heading is greater than or equal to 355, then set the current heading to 0 and add the remaining headChange because cannot have heading over 359.
		//This is ued when user is turning right (Complete 360 turn)
		else if(this.getHeading() >= 355) {
			this.setHeading(0);
